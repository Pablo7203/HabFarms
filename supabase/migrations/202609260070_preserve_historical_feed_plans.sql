-- Legacy rows with no total target keep their original per-bird calculation.
-- Transitioned and newly saved plans use their fixed daily kg target.
...
select p.farm_id,p.flock_id,p.feed_type_id,p.effective_from,p.effective_to,p.grams_per_bird_per_day,
  ...
  round(coalesce(p.daily_feed_kg,
    (fl.initial_birds+coalesce((select sum(case when bm.direction='IN' then bm.quantity else -bm.quantity end) from public.bird_movements bm where bm.flock_id=fl.id and bm.movement_date<=greatest(p.effective_from,fl.start_date)),0))*p.grams_per_bird_per_day/1000
  ),3) target_kg_per_day
...
begin
  ...
  with days as(
    ...
    select d.report_day,fl.id flock_id,
      coalesce(fp.daily_feed_kg,round(public.flock_balance_at(fl.id,d.report_day-1)*fp.grams_per_bird_per_day/1000,3)) target_kg
...
left join lateral (
  ...
  from (
    select coalesce(fp.daily_feed_kg,current_flock.current_live_birds*fp.grams_per_bird_per_day/1000) as daily_kg
    from (
      select distinct on (p.flock_id) p.flock_id,p.feed_type_id,p.grams_per_bird_per_day,p.daily_feed_kg
