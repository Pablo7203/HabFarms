-- Feeding-stage designation is separate from consumption; only effective-dated
-- plan changes update the batch stage. This also removes the unused declaration.
create or replace function public.record_rearing_feed_consumption(target_batch uuid,target_feed_type uuid,target_date date,
...
revoke all on function public.record_rearing_feed_consumption(uuid,uuid,date,numeric,uuid,text,text) from public,anon;
grant execute on function public.record_rearing_feed_consumption(uuid,uuid,date,numeric,uuid,text,text) to authenticated;
