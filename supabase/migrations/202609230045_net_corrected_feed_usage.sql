-- Feed corrections preserve both the original OUT movement and an IN reversal.
-- Net those movements for historical usage and reports so corrected entries do
-- not inflate forecast demand or feed-consumed reporting.
...
create or replace function public.get_feed_report_summary(start_date date,end_date date,target_feed_type uuid default null,target_supplier uuid default null)
returns table(purchased_kg numeric,purchase_value numeric,consumed_kg numeric,consumption_cost numeric,wastage_kg numeric,wastage_cost numeric,supplier_paid numeric,supplier_payable numeric,current_stock numeric,current_inventory_value numeric)
language plpgsql stable security definer set search_path='' as $$
declare f uuid;
begin
  f:=public.reporting_farm(true);
  if $1>$2 then raise exception 'Invalid report date range' using errcode='22023'; end if;
  return query select
    coalesce((select sum(total_kg) from public.feed_purchases where farm_id=f and status='completed' and purchase_date between $1 and $2 and (target_feed_type is null or feed_type_id=target_feed_type) and (target_supplier is null or supplier_id=target_supplier)),0),
    coalesce((select sum(total_cost) from public.feed_purchases where farm_id=f and status='completed' and purchase_date between $1 and $2 and (target_feed_type is null or feed_type_id=target_feed_type) and (target_supplier is null or supplier_id=target_supplier)),0),
    coalesce((select sum(case when direction='OUT' then quantity_kg else -quantity_kg end) from public.feed_inventory_movements where farm_id=f and movement_type in('consumption','consumption_reversal') and movement_date between $1 and $2 and (target_feed_type is null or feed_type_id=target_feed_type)),0),
    coalesce((select sum(case when direction='OUT' then total_cost_snapshot else -total_cost_snapshot end) from public.feed_inventory_movements where farm_id=f and movement_type in('consumption','consumption_reversal') and movement_date between $1 and $2 and (target_feed_type is null or feed_type_id=target_feed_type)),0),
    coalesce((select sum(quantity_kg) from public.feed_inventory_movements where farm_id=f and movement_type='wastage' and movement_date between $1 and $2 and (target_feed_type is null or feed_type_id=target_feed_type)),0),
    coalesce((select sum(total_cost_snapshot) from public.feed_inventory_movements where farm_id=f and movement_type='wastage' and movement_date between $1 and $2 and (target_feed_type is null or feed_type_id=target_feed_type)),0),
    coalesce((select sum(pp.amount) from public.feed_purchase_payments pp join public.feed_purchases fp on fp.id=pp.feed_purchase_id where pp.farm_id=f and pp.voided_at is null and pp.payment_date<=$2 and (target_feed_type is null or fp.feed_type_id=target_feed_type) and (target_supplier is null or fp.supplier_id=target_supplier)),0),
    coalesce((select sum(fp.total_cost-coalesce((select sum(pp.amount) from public.feed_purchase_payments pp where pp.feed_purchase_id=fp.id and pp.voided_at is null and pp.payment_date<=$2),0)) from public.feed_purchases fp where fp.farm_id=f and fp.status='completed' and fp.purchase_date<=$2 and (target_feed_type is null or fp.feed_type_id=target_feed_type) and (target_supplier is null or fp.supplier_id=target_supplier)),0),
    coalesce((select sum(quantity_kg) from public.feed_inventory_balances where farm_id=f and (target_feed_type is null or feed_type_id=target_feed_type)),0),
    coalesce((select sum(inventory_value) from public.feed_inventory_balances where farm_id=f and (target_feed_type is null or feed_type_id=target_feed_type)),0);
end;$$;

revoke all on function public.get_feed_report_summary(date,date,uuid,uuid) from public,anon;
grant execute on function public.get_feed_report_summary(date,date,uuid,uuid) to authenticated;
