...
import { updateSaleCreditTermsAction } from "@/app/actions/sales";
...
export function CreditTermsForm({
  ...
  const [days, setDays] = useState(creditDays ?? 7),
    [due, setDue] = useState(dueDate ?? ""),
    [reason, setReason] = useState(""),
    [message, setMessage] = useState(""),
    [pending, start] = useTransition(),
    router = useRouter();
  const recalc = (v: number) => {
    setDays(v);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        start(async () => {
          const x = await updateSaleCreditTermsAction(saleId, {
            creditDays: days,
            dueDate: due,
            reason,
          });
    ...
    >
          ...
          onChange={(e) => {
            ...
            const delta = Math.round(
              (Date.parse(`${e.target.value}T12:00:00Z`) -
                Date.parse(`${saleDate}T12:00:00Z`)) /
                86400000,
            );
            setDays(delta);
          }}
      ...
      <label className="text-sm font-medium sm:col-span-2">
        ...
        <Input
          ...
          placeholder="Customer requested an extension"
        />
      </label>
      {message && (
