...
export default async function PlatformFarms({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; subscription?: string; page?: string }> }) {
  ...
  return <div>
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">Customer farms</h1><p className="mt-2 text-stone-600">Server-filtered customer account portfolio. Farm operational data remains private.</p></div><div className="flex gap-2"><a href={`/api/platform/export/portfolio?${params.toString()}`} className="inline-flex min-h-11 items-center rounded-xl border px-4 text-sm font-semibold">Export CSV</a><Link href="/platform/farms/new" className="inline-flex min-h-11 items-center rounded-xl bg-emerald-800 px-4 font-semibold text-white">Create farm</Link></div></div>
